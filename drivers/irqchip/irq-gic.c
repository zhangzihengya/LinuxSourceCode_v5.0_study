...
// 针对 GIC-V2 中断控制器的中断处理程序，用于硬件中断号的读取和继续处理中断
...
static const struct irq_domain_ops gic_irq_domain_hierarchy_ops = {
	// 翻译或者转换，通过设备树节点和 DTS 中的中断信息解码出硬件的中断号与中断触发类型
			  ...
			  struct fwnode_handle *handle)
{
	...
	if (handle) {		/* DT/ACPI */
		// irq_domain_create_linear() 函数注册一个 irq_domain 数据结构
