...
// 设置当前进程的状态，set_current_state 在设置进程状态时会考虑 SMP 多核环境下的高速缓存一致性问题
...
struct load_weight {
	...
	u32				inv_weight;			// 全称 inverse weight ，表示权重的中间结算结果
};
...
struct task_struct {
...
#endif
	int				exit_state;
	// 用来存放进程的退出值，这样父进程就可以知道子进程的退出原因
	int				exit_code;
	// 用来存放进程的终止信号，这样父进程就可以知道子进程的退出原因
	int				exit_signal;
	...
	/* Real parent process: */
	// 指向当前进程的父进程的 task_struct 数据结构
	// 如果当前进程的父进程不存在了，则指向进程1（init进程）的进程描述符
	struct task_struct __rcu	*real_parent;

	/* Recipient of SIGCHLD, wait4() reports: */
	// parent 字段 , 指向父进程, 一般情况下与 real_parent 相同;
	// 如果进程被另外一个进程使用系统调用 ptrace 方法跟踪调试了,其父进程就是跟踪进程;
	...
	// 指向当前进程的兄弟进程的链表，链表头在父进程的 sibling 成员中
